...
from .models import *
from .services.progress_advice import update_progress_advice_for_user
...
def topik_home(request):
    ...
    if user.is_authenticated:
        subscription = prepare_subscription(user)
        limits = subscription.get_limits()

        remaining_tests = max(0, limits["tests"] - subscription.tests_used)
        remaining_ai = max(0, limits["ai"] - subscription.ai_used)
    ...
    return render(request, "topik/main.html", {
...
def speaking_test_list(request):
    ...
    context = {
        ...
        # ✅ monthly usage
        "credits_left": max(0, limits["tests"] - subscription.tests_used),
        "ai_left": max(0, limits["ai"] - subscription.ai_used),
